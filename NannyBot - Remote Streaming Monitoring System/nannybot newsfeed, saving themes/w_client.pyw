...
class gui(PySide6.QtCore.QObject):  # not QThread, so this will run as main thread
    ...
    def welcome_phase2(self):
        ...
        json_object = json.dumps(settings, indent=5)
    ...
    def lightTheme(self):
        ...
        with open('settings.json', 'r') as openfile:
            ...
            openfile.close()
        with open('settings.json', 'w') as outfile:
            outfile.write(json.dumps(json_object, indent=5))
            outfile.close()
    ...
    def darkTheme(self):
        ...
        with open('settings.json', 'r') as openfile1:
            json_object = json.load(openfile1)
    ...
    def slot_news(self, feed: str):
        self.newstab = QMessageBox()
        self.newstab.setWindowTitle("Newsfeed")
        self.newstab.setText(feed)
        self.newstab.setStandardButtons(QMessageBox.StandardButton.Close | QMessageBox.StandardButton.NoButton)
        self.newstab.button(QMessageBox.StandardButton.Close).setText('Hide')
        self.newstab.setWindowFlags(Qt.CustomizeWindowHint | Qt.WindowTitleHint)
        self.newstab.move(1220,570)
        self.newstab.setWindowOpacity(0.7)
        self.newstab.raise_()
        self.newstab.open()  # avoids locking screen like "show"
    def slot_showHideNews(self):
        if self.newstab.isHidden():
            self.newstab.open()
            self.newstab.raise_()

            self.showHideNews.setText('Hide')
            self.win.update()
        else:
            self.newstab.hide()
            self.showHideNews.setText('Show')
            self.win.update()
    def ui(self):
        "windows and layout"
        ...
        self.newsMenu = PySide6.QtWidgets.QMenu('News')
        self.showHideNews = PySide6.QtGui.QAction('Hide', self.win)
        self.newsMenu.addAction(self.showHideNews)

        self.showHideNews.triggered.connect(self.slot_showHideNews)
        self.menuBar.addMenu(self.newsMenu)
        """adjust sizes"""
    ...
    def slot_connectAndRun(self, serverIp: str, serverPort:int):
        try:
            ...
            if not self.network.isConnected:
                os.remove(os.path.join(os.path.curdir,"system.json"))
            ...
            while self.on:
                ...
                try:
                    ...
                    pass  # this will infrequently throw an error, especially upon connection
        ...
        except (TimeoutError, socket.error) as exc:  # covers port is taken error for example.
            # must write types in bracket or wont catch latter!

            if os.path.isfile('settings.json'):
                os.remove(os.path.join('','settings.json'))
            
            '''this throw error. enforce with validator?'''
            print(exc)
            traceback.print_tb(exc.__traceback__)  # to show line!
            '''--------------------------------------------------------------------------------'''
            '''this fucks up connect request made onan already connected socket find solution'''
            '''-------------------------------------------------------------------------------'''
            exit(0)
...
if __name__ == '__main__':
    ...
    '''print(socket.gethostbyname(socket.gethostname()))  # ipv4 on netstat
    exit()'''
    ...
    try:
        ...
        '''except below not reached if app.exec'd, add except inside funcs'''
